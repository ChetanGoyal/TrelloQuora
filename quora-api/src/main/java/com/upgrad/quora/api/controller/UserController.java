...
public class UserController {
    ...
    public ResponseEntity<SignupUserResponse> signup(final SignupUserRequest signupUserRequest)
        ...
        SignupUserResponse userResponse = new SignupUserResponse(createdUserEntity.getUuid(),
    ...
    public ResponseEntity<SigninResponse> signin(@RequestHeader("authorization") final String authorization)
        ...
        SigninResponse userResponse = new SigninResponse(user.getUuid(),"SIGNED IN SUCCESSFULLY");
        HttpHeaders headers = new HttpHeaders();
        headers.add("access-token", userAuthToken.getAccessToken());
        return new ResponseEntity<SigninResponse>(userResponse, headers, HttpStatus.OK);
    }

    @RequestMapping(path = "/user/signout", method = RequestMethod.POST,
            produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseEntity<SignoutResponse> signout(@RequestHeader("authorization") final String authorization)
        ...
        SignoutResponse signoutResponse = new SignoutResponse(userEntity.getUuid(),"SIGNED OUT SUCCESSFULLY");
